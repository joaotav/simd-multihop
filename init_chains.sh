...
rm -rf $CHAIN_DIR/$CHAINID_5 &> /dev/null
...
if ! mkdir -p $CHAIN_DIR/$CHAINID_5 2>/dev/null; then
    echo "Failed to create chain5 folder. Aborting..."
    exit 1
fi
...
echo "Initializing $CHAINID_5..."
$BINARY init test --home $CHAIN_DIR/$CHAINID_5 --chain-id=$CHAINID_5
...
echo $VAL_MNEMONIC_5 | $BINARY keys add val5 --home $CHAIN_DIR/$CHAINID_5 --recover --keyring-backend=test
...
echo $WALLET_MNEMONIC_9 | $BINARY keys add wallet1 --home $CHAIN_DIR/$CHAINID_5 --recover --keyring-backend=test
echo $WALLET_MNEMONIC_10 | $BINARY keys add wallet2 --home $CHAIN_DIR/$CHAINID_5 --recover --keyring-backend=test
...
echo $RLY_MNEMONIC_5 | $BINARY keys add rly_chain5 --home $CHAIN_DIR/$CHAINID_5 --recover --keyring-backend=test 
...
$BINARY genesis add-genesis-account $($BINARY --home $CHAIN_DIR/$CHAINID_5 keys show val5 --keyring-backend test -a) 100000000000stake  --home $CHAIN_DIR/$CHAINID_5
...
$BINARY genesis add-genesis-account $($BINARY --home $CHAIN_DIR/$CHAINID_5 keys show wallet1 --keyring-backend test -a) 100000000000stake  --home $CHAIN_DIR/$CHAINID_5
$BINARY genesis add-genesis-account $($BINARY --home $CHAIN_DIR/$CHAINID_5 keys show wallet2 --keyring-backend test -a) 100000000000stake  --home $CHAIN_DIR/$CHAINID_5
...
$BINARY genesis add-genesis-account $($BINARY --home $CHAIN_DIR/$CHAINID_5 keys show rly_chain5 --keyring-backend test -a) 100000000000stake  --home $CHAIN_DIR/$CHAINID_5
...
$BINARY genesis gentx val5 10000000000stake --home $CHAIN_DIR/$CHAINID_5 --chain-id $CHAINID_5 --keyring-backend test
...
$BINARY genesis collect-gentxs --home $CHAIN_DIR/$CHAINID_5
...
sed -i -e 's/timeout_propose = "3s"/timeout_propose = "5s"/g' $CHAIN_DIR/$CHAINID_3/config/config.toml
...
sed -i -e 's/timeout_propose = "3s"/timeout_propose = "5s"/g' $CHAIN_DIR/$CHAINID_4/config/config.toml
...
sed -i -e 's#"tcp://0.0.0.0:26656"#"tcp://0.0.0.0:'"$P2PPORT_5"'"#g' $CHAIN_DIR/$CHAINID_5/config/config.toml
sed -i -e 's#"tcp://127.0.0.1:26657"#"tcp://0.0.0.0:'"$RPCPORT_5"'"#g' $CHAIN_DIR/$CHAINID_5/config/config.toml
sed -i -e 's/timeout_commit = "5s"/timeout_commit = "1s"/g' $CHAIN_DIR/$CHAINID_5/config/config.toml
sed -i -e 's/timeout_propose = "3s"/timeout_propose = "5s"/g' $CHAIN_DIR/$CHAINID_5/config/config.toml
sed -i -e 's#"tcp://0.0.0.0:1317"#"tcp://0.0.0.0:'"$RESTPORT_5"'"#g' $CHAIN_DIR/$CHAINID_5/config/app.toml
sed -i -e 's#":8080"#":'"$ROSETTA_5"'"#g' $CHAIN_DIR/$CHAINID_5/config/app.toml