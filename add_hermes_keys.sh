...
echo $RLY_MNEMONIC_1 | $HERMES_BINARY --config $HERMES_CONFIG_DIR keys add --chain chain1 --mnemonic-file /dev/stdin --key-name rly_wallet --overwrite
...
echo $RLY_MNEMONIC_2 | $HERMES_BINARY --config $HERMES_CONFIG_DIR keys add --chain chain2 --mnemonic-file /dev/stdin --key-name rly_wallet --overwrite
...
echo $RLY_MNEMONIC_3 | $HERMES_BINARY --config $HERMES_CONFIG_DIR keys add --chain chain3 --mnemonic-file /dev/stdin --key-name rly_wallet --overwrite
...
echo $RLY_MNEMONIC_4 | $HERMES_BINARY --config $HERMES_CONFIG_DIR keys add --chain chain4 --mnemonic-file /dev/stdin --key-name rly_wallet --overwrite
